...
if (upgradeStr == null) localStorage.setItem('lastUpgrade', JSON.stringify(upgradeListRef))
else upgradeStr = localStorage.getItem('lastUpgrade')
  ...
  function levelUp(upgradeName: string) {
    ...
    if (index != -1 && upgradeList.value[index] != undefined) {
      ...
      if (Number.parseInt(backup.money) - Number.parseInt(upgradeList.value[index].cost) >= 0) {
        ...
        money.money =
          Number.parseFloat(backup.money) - Number.parseInt(upgradeList.value[index].cost)
      ...
      upgradeList.value[index].cost = (
        Number.parseInt(upgradeList.value[index].cost) * 2
      ).toString()
      saveUpgrade()
    }
  }
